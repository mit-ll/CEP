...
abstract class FIR(busWidthBytes: Int, val c: FIRParams)(implicit p: Parameters)
        ...
        beatBytes = busWidthBytes))
    {
        ...
        lazy val module = new LazyModuleImp(this) {
            ...
            val datain_read_idx         = RegInit(0.U(6.W))
            val datain_read_data        = Wire(UInt(32.W))

            val dataout_write_idx       = RegInit(0.U(6.W))
            val dataout_write_data      = RegInit(0.U(32.W))
            ...
            val dataout_valid           = RegInit(false.B)

            val count                   = RegInit(0.U(6.W))
            val next_out                = Wire(Bool())
