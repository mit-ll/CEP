...
void aes_reportCiphertext(void) {
    ...
    for(unsigned int i = 0; i < AES_BLOCK_WORDS; ++i) {
      printf("%016lx", ct[(AES_BLOCK_WORDS - 1) - i]);
    }
    printf("\n");
}
...
bool dft_readyValid(void) {
    return (cep_read(DFT_BASE_K, DFT_DONE) != 0) ? true : false;
}
...
void dft_setX(uint16_t i, uint16_t pX0, uint16_t pX1, uint16_t pX2, uint16_t pX3){
    cep_write(DFT_BASE_K, DFT_IN_DATA, pX1<<16|pX0);             //Write data
    cep_write(DFT_BASE_K, DFT_IN_ADDR, i);                       //Write addr
    cep_write(DFT_BASE_K, DFT_IN_WRITE, 0x1);                    //Load data
    cep_write(DFT_BASE_K, DFT_IN_WRITE, 0x0);                    //Stop
    
    cep_write(DFT_BASE_K, DFT_IN_DATA+DFT_BLOCK_BYTES, pX3<<16|pX2); //Write data
    cep_write(DFT_BASE_K, DFT_IN_ADDR, i);                       //Write addr
    cep_write(DFT_BASE_K, DFT_IN_WRITE, 0x1);                    //Load data
    cep_write(DFT_BASE_K, DFT_IN_WRITE, 0x0);                    //Stop
}
...
void dft_getY(uint16_t i, uint16_t *pY0, uint16_t *pY1, uint16_t *pY2, uint16_t *pY3) {
    ...
    *pY1=temp>>16;
    ...
    temp=cep_read(DFT_BASE_K, DFT_OUT_DATA+DFT_BLOCK_BYTES); //Read data
    *pY2=temp;
    *pY3=temp>>16;
}
...
bool idft_readyValid(void) {
    return (cep_read(IDFT_BASE_K, IDFT_DONE) != 0) ? true : false;
}
...
void idft_setX(uint16_t i, uint16_t pX0, uint16_t pX1, uint16_t pX2, uint16_t pX3){
    cep_write(IDFT_BASE_K, IDFT_IN_DATA, pX1<<16|pX0);             //Write data
    cep_write(IDFT_BASE_K, IDFT_IN_ADDR, i);                       //Write addr
    cep_write(IDFT_BASE_K, IDFT_IN_WRITE, 0x1);                    //Load data
    cep_write(IDFT_BASE_K, IDFT_IN_WRITE, 0x0);                    //Stop
    
    cep_write(IDFT_BASE_K, IDFT_IN_DATA+IDFT_BLOCK_BYTES, pX3<<16|pX2); //Write data
    cep_write(IDFT_BASE_K, IDFT_IN_ADDR, i);                       //Write addr
    cep_write(IDFT_BASE_K, IDFT_IN_WRITE, 0x1);                    //Load data
    cep_write(IDFT_BASE_K, IDFT_IN_WRITE, 0x0);                    //Stop
}
...
void idft_getY(uint16_t i, uint16_t *pY0, uint16_t *pY1, uint16_t *pY2, uint16_t *pY3) {
    ...
    *pY1=temp>>16;
    ...
    temp=cep_read(IDFT_BASE_K, IDFT_OUT_DATA+IDFT_BLOCK_BYTES); //Read data
    *pY2=temp;
    *pY3=temp>>16;
}
...
bool iir_readyValid(void) {
    return (cep_read(IIR_BASE_K, IIR_DONE) != 0) ? true : false;
}

void iir_start(void) {
    cep_write(IIR_BASE_K, IIR_START, 0x1);
    cep_write(IIR_BASE_K, IIR_START, 0x0);
}
...
void iir_setInData(uint32_t i, uint32_t idata){
    ...
    cep_write(IIR_BASE_K, IIR_IN_WRITE, 0x1);  //Load data
    cep_write(IIR_BASE_K, IIR_IN_WRITE, 0x0);  //Stop
}
...
void md5_strobeMsgValid() {
  cep_write(MD5_BASE_K, MD5_READY, 0x1);
  cep_write(MD5_BASE_K, MD5_READY, 0x0);
}
...
/*Wait for modexp ready signal*/
void rsa_wait_ready(){
    while(cep_read(RSA_ADDR_BASE_K, RSA_ADDR_STATUS)!= 0x00000001);
}
...
void rsa_modexp_32bits(uint32_t Wmsg, uint32_t Wexp, uint32_t Wmod, uint32_t Wres){
    ...
    cep_write(RSA_ADDR_BASE_K, RSA_ADDR_EXPONENT_PTR_RST, 0x00000000);
    cep_write(RSA_ADDR_BASE_K, RSA_ADDR_EXPONENT_DATA   , Wexp);

    cep_write(RSA_ADDR_BASE_K, RSA_ADDR_MODULUS_PTR_RST , 0x00000000);
    cep_write(RSA_ADDR_BASE_K, RSA_ADDR_MODULUS_DATA    , Wmod);

    cep_write(RSA_ADDR_BASE_K, RSA_ADDR_MESSAGE_PTR_RST , 0x00000000);
    cep_write(RSA_ADDR_BASE_K, RSA_ADDR_MESSAGE_DATA    , Wmsg);

    cep_write(RSA_ADDR_BASE_K, RSA_ADDR_EXPONENT_LENGTH , 0x00000001);
    cep_write(RSA_ADDR_BASE_K, RSA_ADDR_MODULUS_LENGTH  , 0x00000001);

    // Start processing and wait for ready.
    cep_write(RSA_ADDR_BASE_K, RSA_ADDR_CTRL            , 0x00000001);
    rsa_wait_ready();

    cep_write(RSA_ADDR_BASE_K, RSA_ADDR_RESULT_PTR_RST  , 0x00000000);
...
void sha256_strobeInit(void) {
    cep_write(SHA256_BASE_K, SHA256_NEXT_INIT, 0x1);
    cep_write(SHA256_BASE_K, SHA256_NEXT_INIT, 0x0);
}

void sha256_strobeNext(void) {
    cep_write(SHA256_BASE_K, SHA256_NEXT_INIT, 0x2);
    cep_write(SHA256_BASE_K, SHA256_NEXT_INIT, 0x0);
}
...
void sha256_loadPaddedMessage(const char* msg_ptr) {
    ...
    for(int i = (SHA256_MESSAGE_BYTES - 1); i >= 0; --i) {
        ...
        if(i % BYTES_PER_WORD == 0) {
            cep_write(SHA256_BASE_K, SHA256_MSG_BASE + i, temp);
        }
    }
}
